...
const TaskCard = (props) => {
    ...
    <div className={`container w-full mx-auto ${props.completed ?'bg-slate-100 dark:bg-slate-700':'bg-amber-50 dark:bg-[#2f2600]'} 
        ...
        <div className="container flex flex-wrap space-x-4 items-center w-fit">
            <div className={`container w-4 h-4 rounded-full inline-block border-2 border-solid border-amber-500 dark:border-amber-400 transition-all ease-in-out duration-400 ${props.completed ? 'opacity-80 bg-amber-500':'hover:bg-amber-400'}`}></div>
        ...
        <TrashIcon className="h-5 w-5 text-slate-500 dark:text-slate-200 hover:text-red-400 dark:hover:text-red-300 transition-all ease-in-out duration-400" aria-hidden="true" onClick={()=>{props.deleteTask()}}/>
    </div>
    </>
    )
}
