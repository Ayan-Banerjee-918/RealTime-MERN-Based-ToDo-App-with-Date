...
const TaskCard = (props) => {
    ...
    return (
    <div className={`container w-full mx-auto ${props.completed ?'bg-slate-200 dark:bg-slate-800':'bg-amber-50 dark:bg-[#302e46]'} 
        ...
        <div className="container flex flex-wrap space-x-4 items-center w-fit">
            <div className={`container w-4 h-4 rounded-full inline-block border-2 border-solid  border-amber-600 dark:border-amber-400 transition-all ease-in-out duration-400 ${props.completed ? 'opacity-80 bg-amber-500 hover:bg-slate-100 dark:hover:bg-slate-600':'hover:bg-amber-400'}`} onClick={()=>{props.completeTask()}}></div>
            <div className="container sm:w-fit w-52 flex-col flex">
                <span className={`font-medium text-sm sm:text-base text-amber-700 dark:text-[#FDC57B] ${props.completed?'line-through':''}`}>{props.title}</span>
                <div className='flex gap-2'>    
                <span className="text-xs text-slate-500 dark:text-slate-300 font-medium">{dueDate}</span>
                { !props.completed && new Date(props.due) < today ? (<span className="text-xs text-red-500 dark:text-red-300 font-medium">Overdue</span>): null}
                </div>
            </div>
        </div>
